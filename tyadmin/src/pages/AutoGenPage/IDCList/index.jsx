...
import {addIDC, queryIDC, removeIDC, updateIDC} from './service';
...
const TableList = () => {
  ...
  try {
    await updateIDC(value, current_id);
    hide();
  ...
  const base_columns = [{
          ...
          return dealDateTimeDisplay(text);
              ...
              const children = serversManyToManyList.map(item => {
                return (
              ...
              text.forEach((value, index, arr) => {
                child.push(<Tag color={color_arr[value % 10]}>{serversManyToManyMap[value]}</Tag>);
              });
  ...
  const [paramState, setParamState] = useState({});
    ...
    const [serversManyToManyList, setServersManyToManyList] = useState([]);
                      const [serversManyToManyMap, setServersManyToManyMap] = useState([]);
                    useEffect(() => {
                      queryHosts().then(value => {
                        setServersManyToManyList(value.data);
                        let getServersManyToManyMap = {};
              for (let index in value.data) {
                let item = value.data[index];
                getServersManyToManyMap[item.id.toString()] = item.name;
              }
              setServersManyToManyMap(getServersManyToManyMap);
                      });
                    }, []);
  return (
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索机房 " onSearch={value => {
            setParamState({
              search: value,
            });
